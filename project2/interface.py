...
class GUI:
    ...
    def run(self):
        ...
        #button
        self.connect_btn = ttk.Button(conn_frame, text="Connect", command=self._mock_connect_db)
        self.connect_btn.grid(row=0, column=10, padx=10)
        
        # Query input block
        query_frame = ttk.LabelFrame(main_frame, text="SQL Query", padding="5")
        query_frame.grid(row=1, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), pady=5)
